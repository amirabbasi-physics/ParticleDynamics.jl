@testset "GPU Residency and Backend Smoke" begin
    ...
    @testset "CUDA-backed state and OU buffers stay on device" begin
        ...
        @test SimulationCore.backend(st) isa ParticleDynamics.Backends.CUDABackend
        ...
        spec = SimulationCore.velocityverlet(st; gamma=1f0, temperature=0.5f0, dt=dt)
    ...
    @testset "Thermostat workspaces stay on device" begin
        ...
        nhc = SimulationCore.nosehooverchain(st_nhc; temperature=0.5f0, tau=0.25f0, chain_length=4, substeps=3)
        ...
        SimulationCore.step!(st_nhc, nhc.params, dt; compute_energy=false)
        ...
        csvr = SimulationCore.csvr(st_csvr; temperature=0.5f0, tau=0.25f0)
        ...
        SimulationCore.step!(st_csvr, csvr.params, dt; compute_energy=false)
