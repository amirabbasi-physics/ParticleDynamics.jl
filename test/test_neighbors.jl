@testset "Neighbor Lists" begin
    ...
    @test st.nbh isa NonEqSimGPU.NeighborLists.NeighborMatrix{Float32}

    needed0 = NonEqSimGPU.NeighborLists.update_needed!(
        st.nbh, st.rx, st.ry;
    ...
    rx[1] += 0.8f0
    copyto!(st.rx, rx)
    needed1 = NonEqSimGPU.NeighborLists.update_needed!(
        st.nbh, st.rx, st.ry;
    ...
    @test st_ap.nbh isa NonEqSimGPU.NeighborLists.AllPairsNeighborMatrix{Float32}
