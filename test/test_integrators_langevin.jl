@testset "Langevin Integrators" begin
    @testset "Correlated noise path" begin
        ...
        spec = Simulation.velocityverlet(st; gamma=1f0, temperature=1f0, noise_corr_time=0.05f0, dt=dt)
    ...
    @testset "BAOAB/BAOA/GSM smoke" begin
        ...
        for spec in (Simulation.baoab(st; gamma=1f0, temperature=1f0, dt=dt),
                     Simulation.baoa(st; gamma=1f0, temperature=1f0, dt=dt),
                     Simulation.gsm(st; gamma=1f0, temperature=1f0, dt=dt))
    ...
    @testset "Freeze hold" begin
        ...
        vv = Simulation.velocityverlet(st; gamma=0f0, temperature=0f0, dt=dt)
        rx0 = Filters.gather(st.rx, frozen)
        ry0 = Filters.gather(st.ry, frozen)

        for _ in 1:2
            Simulation.step!(st, vv, dt; compute_energy=false)
