...
# Choose one type of bonded interaction (harmonic or FENE)
use_fene = true
bonding = use_fene ? fene_bond(k = 300.0, r0 = 1.5) : harmonic_bond(k = 300f0, r0 = 1.0)
...
@time for s in 1:N_steps
    if s % N_log == 0
        step!(st, dt)
        write_observables_csv!(joinpath(@__DIR__, "obs3d_polymer.csv"), s; Epot=st.Epot, Ekin=st.Ekin, dq=st.dq)
        write_gsd_frame!(gsdh, st; diameter=sigma, types_names=types, step=st.step)
