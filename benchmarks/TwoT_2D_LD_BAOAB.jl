...
function main()
    ...
    soft_sigma = 1.0f0
    soft_epsilon = 5.0f8
    rcut = soft_sigma              # r < σ for harmonic repulsion
    ...
    nzero = Base.count(==(0f0), noise_scale)
    @info "Noise scale" cold_first=noise_scale[cold_idx[1]] hot_first=noise_scale[hot_idx[1]] mean_cold=mean_ns_cold mean_hot=mean_ns_hot zeros=nzero

    # Integrator info (explicit)
    println("Integrator: Langevin dynamics (BAOAB splitting).")
    println(" - Sequence: B(1/2) → A(1/2) → O(Δt) → A(1/2) → B(1/2)")
    ...
    if isempty(records)
        ...
        println("Recorded $(length(records)) BAOAB samples for cold/hot groups")
